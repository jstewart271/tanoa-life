...
*/
params [["_className", "", [""]], ["_colorIdx", 0, [0]], ["_IDD", 3700, [0]], ["_RSC", "RscPreview", [""]]];

if !(isNull (findDisplay _IDD)) exitWith {}; // Already init'd
if (_className isEqualTo "") exitWith { ["You must select a vehicle to preview first..."] spawn PHX_fnc_notify };
...
if (_RSC isEqualTo "RscPreview") then {
    _display displaySetEventHandler ["keyDown", "[] call life_fnc_3dPreviewExit; true"]; // Rebind escape...
};
...
PHX_Hud_Setting = PHX_hideHUD; // Save it...
PHX_hideHUD = true;
[] call PHX_fnc_hudUpdate;

[_className, _colorIdx, _IDD] call life_fnc_3dPreviewDisplay;