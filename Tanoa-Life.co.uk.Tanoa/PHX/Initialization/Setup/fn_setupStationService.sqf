...
{
   private _pump = nearestObjects [_x, ["Land_fs_feed_F","Land_FuelStation_01_pump_F","Land_FuelStation_02_pump_F","Land_FuelStation_Feed_F","Land_FuelStation_03_pump_F","Land_FuelStation_01_pump_malevil_F"], 100];

    {
        _x setFuelCargo 0;
        _x addAction ["Refuel Vehicle", life_fnc_fuelStatOpen, 1, 3, true, true, "", '_this distance _target < 5 && cursorObject isEqualTo _target'];
        _x addAction ["Deliver Petroleum", PHX_fnc_deliverPetrol, 1, 3, true, true, "", '_this distance _target < 5 && cursorObject isEqualTo _target && !(isNil "PHX_petrolDest")'];
    } forEach _pump;
