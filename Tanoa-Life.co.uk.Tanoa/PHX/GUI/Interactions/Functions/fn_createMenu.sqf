...
if (dialog || { isRestrained(player) }) exitWith {};
if !(canSuspend) exitWith { _this spawn PHX(createMenu) };
...

{
	...
	PHX_createdElements pushBack _button; // Save it to be deleted later...
} forEach _buttons;
