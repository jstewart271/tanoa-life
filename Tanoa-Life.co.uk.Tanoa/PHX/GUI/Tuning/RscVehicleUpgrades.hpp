class RscVehicleUpgrades {
    idd = 33000;
    name = "RscVehicleUpgrades";
    movingEnable = 0;
    enableSimulation = 1;
	onLoad = "['onLoad'] spawn PHX_fnc_openVehicleUpgrades;";
	...
	class Controls {
		class Listbox: Life_RscListBox {
			...
			font = "RobotoCondensedLight";
			sizeEx = (((((safezoneW / safezoneH) min 1.2) / 1.2) / 30) * 1);
			onLBSelChanged = "['lbChange'] call PHX_fnc_openVehicleUpgrades";
		};
		...
		class Upgrade: Life_RscButton {
			...
			font = "PuristaLight";
			sizeEx = (((((safezoneW / safezoneH) min 1.2) / 1.2) / 30) * 1);
			onButtonClick = "[] spawn PHX_fnc_purchaseVehicleUpgrade";
		};
